...
export class VideoEditorConfig implements EditorPlugin {
  ...
  async initialize({ cesdk, engine }: EditorPluginContext) {
    if (cesdk) {
      ...
      // Set the user role via runtime API
      // This configures scopes and settings based on the Creator role
      ...
      // These checks verify that the browser supports video encoding/decoding.
      // - videoDecode: 'block' - Block if browser can't decode video
      // - videoEncode: 'warn' - Warn if browser can't encode video
